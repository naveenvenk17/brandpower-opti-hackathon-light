...
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), settings.DATA_DIR)
...
def apply_grouped_edits(full_df: pd.DataFrame, edited_df: pd.DataFrame, edited_columns: List[str]) -> pd.DataFrame:
    ...
    id_cols = [col for col in lst_id_columns if col in edited_df.columns and col in full_df.columns]
    
    if not id_cols:
        logger.warning("No common ID columns found, returning full_df unchanged")
        return full_df
    
    # Identify value columns that were edited (exclude ID columns)
    value_cols = [col for col in edited_columns if col not in id_cols and col in edited_df.columns]
...
def get_service_and_baseline():
    ...
    if _service_cache is None:
        ...
        logger.info(f"AutoGluon forecast service initialized with model: {model_path}")
    
    if _baseline_forecast_cache is None:
        # Load baseline forecast from CSV for initial display
        baseline_csv_path = os.path.join(DATA_DIR, "baseline_forecast.csv")
        logger.info("Loading baseline forecast CSV for initial display: %s", baseline_csv_path)
        ...
        logger.info(f"Baseline forecast cached: {len(_baseline_forecast_cache)} rows")
...
async def analysis(request: Request):
    ...
    ctx: Dict[str, Any] = {"request": request, "selected_country": selected_country}
    ...
    try:
        ...
        logger.info(f"Detected columns - brand: {brand_col}, year: {year_col}, month: {month_col}, week: {week_col}")
        ...
        if year_col and month_col:
            ...
            logger.info(f"Filtered data: {original_count} -> {len(df)} rows (2024-07 onwards)")
        
        # Extract filter values BEFORE converting to dict
        if brand_col:
            brands = sorted([str(x) for x in df[brand_col].dropna().unique().tolist()])
            logger.info(f"Extracted {len(brands)} brands: {brands[:5]}...")
        
        if year_col:
            years = sorted([int(x) for x in pd.to_numeric(df[year_col], errors="coerce").dropna().unique().tolist()])
            logger.info(f"Extracted years: {years}")
        
        if month_col:
            months = sorted([int(x) for x in pd.to_numeric(df[month_col], errors="coerce").dropna().unique().tolist()])
            logger.info(f"Extracted months: {months}")
        
        if week_col:
            weeks = sorted([int(x) for x in pd.to_numeric(df[week_col], errors="coerce").dropna().unique().tolist()])
        ...
        logger.info(f"Prepared {len(table_rows)} rows for display with {len(table_columns)} columns")
...
def forecast_baseline(country: str, brand: str, max_horizon: int = 4):
    ...
    try:
        logger.info(f"Baseline forecast request: country={country}, brand={brand}")
...
def simulate_scenario(req: SimulateRequest):
    ...
    try:
        logger.info(f"Simulate scenario request for brands: {req.target_brands}")
        logger.info(f"Input: {len(req.edited_rows)} rows, {len(req.columns)} columns")
        ...
        if not uploaded_file_path or not os.path.exists(uploaded_file_path):
            logger.warning("No uploaded file found, using baseline forecast only")
            # Return baseline as both baseline and simulated
            baseline_data = {}
            simulated_data = {}
            
            for brand_name in req.target_brands:
                brand_country = edited_df[edited_df['brand'] == brand_name]['country'].iloc[0].lower()
                brand_baseline_df = baseline_forecast[
                    (baseline_forecast['country'] == brand_country) & 
                    (baseline_forecast['brand'] == brand_name.upper())
                ].sort_values(['year', 'quarter'])
                power_values = brand_baseline_df['predicted_power'].head(req.max_horizon).tolist()
                baseline_data[brand_name] = power_values
                simulated_data[brand_name] = power_values  # Same as baseline when no file
        ...
        try:
            ...
            logger.info(f"Simulation completed for {len(simulated_data)} brands")
            return {
                'baseline': baseline_data,
                'simulated': simulated_data,
                'quarters': quarters
            ...
            logger.error(f"AutoGluon forecast failed: {forecast_error}", exc_info=True)
            ...
            for brand_name in req.target_brands:
                ...
                power_values = brand_baseline_df['predicted_power'].head(req.max_horizon).tolist()
                baseline_data[brand_name] = power_values
                simulated_data[brand_name] = power_values  # Same as baseline on error
...
def optimize_allocation(req: OptimizeRequest):
    ...
    try:
        ...
        if req.method == "equal":
            ...
            allocation = {channel: allocation_per_channel for channel in channels}
        else:
...
def api_optimizer_ga(req: GAOptimizerRequest):
    ...
    try:
        ...
        if not uploaded_file_path:
            ...
            raise HTTPException(status_code=400, detail="No uploaded file found in session")
        ...
        if total_spend <= 0:
            try:
                ...
                real_cols = [lower_map.get(gc.lower(), None) for gc in group_cols]
                real_cols = [c for c in real_cols if c]
                total_spend = float(df_view[real_cols].fillna(0).sum().sum()) if real_cols else 0.0
                logger.info(f"Derived default budget from dataset: {total_spend:,.2f}")
            except Exception as budget_err:
                logger.warning(f"Could not derive budget from data: {budget_err}")
                total_spend = 0.0

        # Get megabrands list (use provided or default to Colombia megabrands)
        megabrands_list = req.megabrands if req.megabrands else list(colombia_megabrands)
        ...
        for brand in megabrands_list:
            ...
            if not brand_baseline_df.empty:
                baseline[brand] = brand_baseline_df['predicted_power'].head(4).tolist()
            else:
        ...
        try:
            ...
            scored['quarter'] = scored['week'].apply(lambda x: week_to_quarter(int(x)))
            ...
            for b in megabrands_list:
                ...
                arr = []
                for q in quarters:
                    vals = bdf[bdf['quarter'] == q]['power'] if 'power' in bdf.columns else pd.Series(dtype=float)
...
def api_brand_power_optimizer(req: Dict[str, Any]):
    ...
    try:
        ...
        # Extract parameters
        total_budget = float(req.get('total_budget', req.get('amount', 1_000_000_000)))
        brands = req.get('brands', list(colombia_megabrands))
        quarters = req.get('quarters', ['2024 Q3', '2024 Q4', '2025 Q1', '2025 Q2'])
        ...
        try:
            logger.info("")
            logger.info("🎯 ATTEMPTING PRIMARY OPTIMIZER: Brand Power Optimizer")
            logger.info("-" * 80)
            
            # Load baseline forecast
            baseline_forecast_path = os.path.join(DATA_DIR, 'baseline_forecast.csv')
            if not os.path.exists(baseline_forecast_path):
                logger.warning(f"⚠️  Baseline forecast not found at: {baseline_forecast_path}")
            ...
            logger.info(f"✓ Power predictor initialized with {len(predictor._baseline_power_cache)} brands")
            ...
            constraints = OptimizationConstraints(
                ...
                wholesalers_min_pct=float(constraint_dict.get('wholesalers_min_pct', 0.0)),
                wholesalers_max_pct=float(constraint_dict.get('wholesalers_max_pct', 1.0)),
                per_brand_min_budget=float(constraint_dict.get('per_brand_min_budget', 0.0)),
                per_brand_max_budget=float(constraint_dict.get('per_brand_max_budget', 0)) or None,
            )
            logger.info(f"✓ Constraints configured: PayTV cap = {paytv_max_pct*100}%")
            ...
            # Run optimization
            logger.info(f"▶ Running Brand Power Optimizer ({method} method)...")
            result = optimizer.optimize(opt_request)
            
            if result.success:
                logger.info("")
                logger.info("✅ PRIMARY OPTIMIZER SUCCEEDED!")
                logger.info(f"   Power Uplift: +{result.total_uplift_pct:.2f}%")
                logger.info(f"   Baseline: {result.total_baseline_power:.2f}")
                logger.info(f"   Optimized: {result.total_optimized_power:.2f}")
                logger.info(f"   Constraints: {'✓ Satisfied' if result.constraints_satisfied else '✗ Violations'}")
                logger.info("="*80)
                ...
                raise ValueError("Brand Power Optimizer did not converge successfully")
            ...
            if not uploaded_file_path or not os.path.exists(uploaded_file_path):
                logger.error("❌ FALLBACK ALSO FAILED: No uploaded data file found")
                logger.error("   Cannot run GA optimizer without historical data")
                logger.error("="*80)
                raise HTTPException(
                    status_code=500, 
            ...
            logger.warning(f"   ⚠️  Used GA fallback due to: {str(primary_error)}")
            logger.info("="*80)
        ...
        logger.error("!"*80)
        logger.exception("Full traceback:")
        raise HTTPException(status_code=500, detail=f"All optimizers failed: {str(e)}")
...
def calculate(req: Dict[str, Any]):
    ...
    try:
            ...
            try:
                ...
                if quarterly_df is not None and not quarterly_df.empty:
                    ...
                    historical_mask = (
                        ...
                        ((quarterly_df['year'] == 2024) & (quarterly_df['quarter'].isin(['Q1', 'Q2'])))
                    )
                    ...
                    if 'power' in historical_quarterly.columns and 'brand' in historical_quarterly.columns:
                        ...
                        unique_quarters = historical_quarterly[['year', 'quarter']].drop_duplicates().sort_values(['year', 'quarter'])
                        historical_quarters = [f"{row['year']} {row['quarter']}" for _, row in unique_quarters.iterrows()]
                        ...
                        for brand in historical_quarterly['brand'].unique():
                            ...
                            avg_power_per_quarter = brand_hist.groupby(['year', 'quarter'])['power'].mean().sort_index()
                            
                            # Convert to list in chronological order matching historical_quarters
                            historical_data[brand] = [
                                float(avg_power_per_quarter.get((int(q.split()[0]), q.split()[1]), 0.0))
                                for q in historical_quarters
                            ]

                logger.info(f"Calculated historical data for {len(historical_data)} brands across {len(historical_quarters)} quarters")
        ...
        logger.info(f"Calculate endpoint: received {len(edited_df)} rows with {len(columns)} columns")
        ...
        if not uploaded_file_path or not os.path.exists(uploaded_file_path):
            logger.warning(f"Uploaded file not found: {uploaded_file_path}, using baseline forecast only")
            # Return baseline as both baseline and simulated
            ...
            for brand_name in target_brands:
                brand_country = edited_df[edited_df['brand'] == brand_name]['country'].astype(str).iloc[0].lower()
                brand_baseline_df = baseline_forecast[
                    (baseline_forecast['country'] == brand_country) & 
                    (baseline_forecast['brand'] == brand_name.upper())
                ].sort_values(['year', 'quarter'])
                power_values = brand_baseline_df['predicted_power'].head(4).tolist()
                baseline_data[brand_name] = power_values
                simulated_data[brand_name] = power_values  # Same as baseline
        else:
            # Load full dataset with all features
            full_df = pd.read_csv(uploaded_file_path)
            logger.info(f"Loaded full original data: {full_df.shape}")
            logger.info(f"Original columns: {full_df.columns.tolist()}")
            
            # Apply user edits to the full dataset
            # The edited_df contains user-modified values for specific columns
            # We need to merge these changes into the full dataset
            full_df = apply_grouped_edits(full_df, edited_df, columns)
            ...
            try:
                ...
                for brand_name in target_brands:
                    ...
                    if not brand_baseline_df.empty:
                        ...
                        logger.warning(f"Baseline not found for brand: {brand_name}")
                
                logger.info(f"Forecast completed: {len(simulated_data)} brands")
...
def export_experiments_excel():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        for i, exp in enumerate(experiments):
            ...
            all_brands = set(list(baseline_data.keys()) + list(simulated_data.keys()))
                    ...
                    for group_name in channel_groups_dict.keys():
                        ...
                        if isinstance(table_rows, list):
                            for table_row in table_rows:
                                row_brand = table_row.get('brand', '') or table_row.get('Brand', '')
                            ...
                            if brand in table_rows:
                                ...
                                if isinstance(brand_data, dict) and group_name in brand_data:
                                    group_total = float(brand_data.get(group_name, 0))
            ...
            if export_data:
                ...
                # Fallback if no data
                empty_df = pd.DataFrame({'Message': ['No data available for this experiment']})
